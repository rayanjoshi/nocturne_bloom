...
class StockDataset(Dataset):
    ...
    def generate_windows(self, dataFrame, cfg: DictConfig):
        ...
        logger.info(f"Target range before scaling: [{y.min():.6f}, {y.max():.6f}]")
        ...
        logger.info(f"Train targets range: [{y_train.min():.6f}, {y_train.max():.6f}]")
        logger.info(f"Val targets range: [{y_val.min():.6f}, {y_val.max():.6f}]")
        ...
        logger.info(f"Target scaler saved to: {target_scaler_path.absolute()}")
        ...
        logger.info(f"Scaled data saved to: {x_save_path}.npy and {y_save_path}.npy")
    ...
    def save_tensors(self, cfg: DictConfig):
        ...
        logger.info(f"  Y: {y_save_path.absolute()}")
        logger.info(f"X tensor shape: {x_tensor.shape}")
        logger.info(f"Y tensor shape: {y_tensor.shape}")
...
class StockDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        logger.info(f"  Y: {y_path.absolute()}")
