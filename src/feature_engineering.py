...
def feature_engineering(df, save_data_path):
    ...
    logger = log_function_start(
        "feature_engineering", df=df, save_data_path=save_data_path
    )
    ...
    if nan_count > 0:
        ...
        logger.warning(
            f"Found {nan_count} NaN values in initial df ({nan_pct:.2f}% of all values)"
        )
    ...
    new_features["Doji"] = (abs(df["Close"] - df["Open"]) / df["Close"] < 0.001).astype(
        int
    )
    logger.debug(
        f"Created 'Doji' feature: {new_features['Doji'].sum()} Doji candles detected"
    )
    ...
    logger.debug(
        f"Created 'Bullish_Engulfing': {count_bullish_engulf} patterns detected"
    )
    ...
    logger.debug(
        f"Created 'Bearish_Engulfing': {count_bearish_engulf} patterns detected"
    )
    ...
    new_features["Hammer"] = (
        ...
        & (
            new_features["lowerWick"] > 0.5 * new_features["Body"].abs()
    ...
    new_features["Shooting_Star"] = (
        ...
        & (
            new_features["upperWick"] > 0.5 * new_features["Body"].abs()
        )  # Upper wick dominates
    ).astype(int)
    logger.debug(
    ...
    logger.debug(
        f"Created 'Bullish_Trend' with mean: {new_features['Bullish_Trend'].mean():.2f}"
    )
    new_features["Bearish_Trend"] = (df["Close"] < df["Open"]).rolling(5).mean()
    logger.debug(
        f"Created 'Bearish_Trend' with mean: {new_features['Bearish_Trend'].mean():.2f}"
    )
    ...
    logger.debug(
        f"Created 'Wick_to_Body_Ratio' with mean: {mean_wbr:.2f}, std: {std_wbr:.2f}"
    )
    ...
    new_features["RSI"] = ta.momentum.RSIIndicator(df["Close"], window=14).rsi()
    ...
    new_features["MACD"] = macd_data.macd()
    new_features["MACDSignal"] = macd_data.macd_signal()
    ...
    if len(df) < 200:
        logger.error(
            f"Insufficient data for SMA_200 calculation: {len(df)} rows available"
        )
    ...
    new_features["massIndex"] = ta.trend.MassIndex(df["High"], df["Low"]).mass_index()

    new_features["commodityChannelIndex"] = ta.trend.CCIIndicator(
        df["High"], df["Low"], df["Close"], window=20
    ).cci()

    new_features["price_acceleration"] = df["Close"].pct_change(5) - df[
        "Close"
    ].pct_change(10)
    ...
    new_features["momentum_consistency"] = (new_features["momentum_1d"] > 0).rolling(
        5
    ).sum() / 5
    ...
    new_features["Stochastic_K"] = stoch.stoch()
    new_features["Stochastic_D"] = stoch.stoch_signal()
    ...
    logger.debug(f"Created 'Stochastic_K' mean {k_mean:.2f} std {k_std:.2f}")
    logger.debug(f"Created 'Stochastic_D' mean {d_mean:.2f} std {d_std:.2f}")

    new_features["Price_RSI_Divergence"] = new_features["momentum_5d"] - new_features[
        "RSI"
    ].diff(5)
    logger.debug(
    ...
    new_features["Reversal_Score"] = (
        rsi_buy - rsi_sell + macd_pos - macd_neg + stoch_buy - stoch_sell
    )
    min_rs = new_features["Reversal_Score"].min()
    max_rs = new_features["Reversal_Score"].max()
    logger.debug(f"Created 'Reversal_Score' with range: [{min_rs:.2f}, {max_rs:.2f}]")

    # --------- Volatility Indicators --------- #
    bb_data = ta.volatility.BollingerBands(df["Close"], window=20, window_dev=2)
    new_features["BBHigh"] = bb_data.bollinger_hband()
    new_features["BBLow"] = bb_data.bollinger_lband()
    ...
    new_features["averageTrueRange"] = ta.volatility.AverageTrueRange(
        df["High"], df["Low"], df["Close"], window=14
    ).average_true_range()
    # Normalize ATR change by price to avoid a long single line
    atr_ratio = new_features["averageTrueRange"] / df["Close"]
    new_features["ATR_Normalized_Change"] = (
        new_features["closeChangePercentage"] / atr_ratio
    )
    mean_atr = new_features["ATR_Normalized_Change"].mean()
    std_atr = new_features["ATR_Normalized_Change"].std()
    logger.debug(
        f"Created 'ATR_Normalized_Change' with mean: {mean_atr:.2f}, std: {std_atr:.2f}"
    )

    new_features["rollingStd"] = df["Close"].rolling(window=14).std()
    aroon_data = ta.trend.AroonIndicator(df["High"], df["Low"], window=14)
    new_features["aroonUp"] = aroon_data.aroon_up()
    new_features["aroonDown"] = aroon_data.aroon_down()
    new_features["ulcerIndex"] = ta.volatility.UlcerIndex(
        df["Close"], window=14
    ).ulcer_index()
    ...
    logger.debug(
        f"Created 'vix_regime': {int(new_features['vix_regime'].sum())} high VIX periods"
    )

    vol_up_threshold = new_features["BBHigh"] + new_features["averageTrueRange"]
    new_features["Volatility_Breakout_Up"] = (df["Close"] > vol_up_threshold).astype(
        int
    )
    ...
    vol_breakout_down = df["Close"] < (
        new_features["BBLow"] - new_features["averageTrueRange"]
    )
    ...
    new_features["volumeRateOfChange"] = (
        (df["Volume"] - df["Volume"].shift(12)) / df["Volume"].shift(12) * 100
    )

    new_features["onBalanceVolume"] = ta.volume.OnBalanceVolumeIndicator(
        df["Close"], df["Volume"]
    ).on_balance_volume()
    ...
    new_features["chaikinMoneyFlow"] = cmf.chaikin_money_flow()
    ...
    logger.debug(f"Created 'forceIndex' with mean: {fi_mean:.2f}, std: {fi_std:.2f}")

    new_features["Volume_Breakout_Up"] = (
        (df["Close"] > new_features["BBHigh"]) & (new_features["volume_surge"] > 1.5)
    ).astype(int)
    logger.debug(
        f"Created 'Volume_Breakout_Up': {new_features['Volume_Breakout_Up'].sum()} events"
    )
    new_features["Volume_Breakout_Down"] = (
        (df["Close"] < new_features["BBLow"]) & (new_features["volume_surge"] > 1.5)
    ).astype(int)
    ...
    logger.debug(
        "Created 'negativeVolumeIndex' with mean: %.2f, std: %.2f", mean_nvi, std_nvi
    )
    ...
    new_features["vol_percentile_21d"] = (
        new_features["rolling_vol_21"].rolling(126).rank(pct=True)
    )
    new_features["vol_percentile_5d"] = (
        new_features["rolling_vol_5"].rolling(63).rank(pct=True)
    )
    new_features["regime_high_vol"] = (new_features["vol_percentile_21d"] > 0.8).astype(
        int
    )
    ...
    # --------- Gap Analysis --------- #
    new_features["overnight_gap"] = (df["Open"] - df["Close"].shift(1)) / df[
        "Close"
    ].shift(1)
    new_features["gap_magnitude_avg_5d"] = (
        new_features["overnight_gap"].abs().rolling(5).mean()
    )
    new_features["large_gap_frequency"] = (
        (new_features["overnight_gap"].abs() > 0.03).rolling(20).sum()
    )
    logger.debug(
    ...
    new_features["tech_leadership"] = (qqq_pct - spy_pct).rolling(10).mean()

    new_features["semiconductor_leadership"] = (soxx_pct - qqq_pct).rolling(10).mean()

    new_features["nvda_outperformance"] = (nvda_pct - soxx_pct).rolling(5).mean()
    new_features["sector_momentum_divergence"] = df["Close"].pct_change(10) - df[
        "SOXX_Close"
    ].pct_change(10)
    ...
    new_features["momentum_persistence_3d"] = (
        new_features["nvda_outperformance"] > 0
    ).rolling(3).sum() / 3
    ...
    logger.debug(
        f"Created 'ai_momentum_strength' with range: [{ai_min:.2f}, {ai_max:.2f}]"
    )
    ...
    logger.debug(
        f"Created 'momentum_acceleration' with mean: {mean_ma:.2f}, std: {std_ma:.2f}"
    )

    new_features["confirmed_momentum"] = new_features["ai_momentum_strength"] * (
        new_features["volume_surge"] > 1
    ).astype(float)
    ...
    new_features["spy_nvda_correlation"] = nvda_pct_change.rolling(20).corr(
        spy_pct_change
    )

    qqq_pct_change = df["QQQ_Close"].pct_change()
    new_features["qqq_nvda_correlation"] = nvda_pct_change.rolling(20).corr(
        qqq_pct_change
    )
    new_features["treasury_equity_spread"] = df["Treasury_10Y"].diff() * -1

    new_features["Sector_Direction_Divergence"] = new_features["momentum_5d"] - df[
        "SOXX_Close"
    ].pct_change(5)
    ...
    new_features["VIX_Direction_Signal"] = (df["VIX_Proxy"] > vix_thresh).astype(int)
    logger.debug(
    ...
    new_features["trend_strength"] = (
        (df["Close"] > new_features["SMA_200"]).rolling(20).mean()
    )
    ...
    for i in range(len(df)):
        ...
        if (
            new_features["breakout_signal"].iloc[i] == 1
            or new_features["Volatility_Breakout_Up"].iloc[i] == 1
            or new_features["Volatility_Breakout_Down"].iloc[i] == 1
        ):
            last_breakout_idx = i
    ...
    logger.debug(
        f"Created 'Time_Since_Breakout' with mean: {np.mean(time_since_breakout):.2f}"
    )
    ...
    atr_multiplier = 1.5
    atr_thresh = (
        new_features["averageTrueRange"].rolling(atr_window).mean() * atr_multiplier
    )
    ...
    new_features["Direction_Target"] = np.where(
        new_features["direction_pct"] > hybrid_thresh,
        2,
    ...
    if nan_count > 0:
        nan_pct = (nan_count / total_values) * 100
        logger.warning(
            f"Found {nan_count} NaN values in final df ({nan_pct:.2f}% of all values)"
        )
...
@hydra.main(
    version_base=None, config_path="../configs", config_name="feature_engineering"
)
def main(cfg: Optional[DictConfig] = None):
    ...
    try:
        ...
        save_data_path = Path(
            repo_root / cfg.features.preprocessing_data_path
        ).resolve()
